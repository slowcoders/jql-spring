...
//    @PostConstruct
        ...
        for (Method m : getClass().getDeclaredMethods()) {
            if (m.getName().equals("getEntityId")) {
                ...
                if (ret_t != Object.class) {
                    ...
                    List<Field> idFields = JPAUtils.findIdFields(entityType);
                    this.pkNames = new String[idFields.size()];
                    int idx = 0;
                    for (Field f : idFields) {
                        pkNames[idx++] = f.getName();
                        this.hasGeneratedId |= f.getAnnotation(GeneratedValue.class) != null;
                    }
                    return;
                }
            }
    ...
    private Predicate build_where(Root<?> root, Object filter, CriteriaQuery<?> query) {
        ...
        if (filter instanceof Map) {
            ...
            if (pkNames.length == 1) {
                filterMap.put(this.pkNames[0] + "@eq", filter);
            }
            else if (filter.getClass().isArray()) {
                Object values[] = (Object[]) filter;
                if (values.length != pkNames.length) {
                    throw new RuntimeException("invalid primary key: " + values);
                }
                for (int i = 0; i < pkNames.length; i ++) {
                    filterMap.put(pkNames[i] + "@eq", values[i]);
                }
            }
            else {
                if (idType != filter.getClass()) {
                    throw new RuntimeException("invalid id: " + filter);
                }
                try {
                    for (Field f : idType.getDeclaredFields()) {
                        f.setAccessible(true);
                        filterMap.put(f.getName() + "@eq", f.get(filter));
                    }
